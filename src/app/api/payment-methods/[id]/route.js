...
export async function GET(request, { params }) {
  try {
    ...
    const methods = await query('SELECT * FROM payment_methods WHERE id = ?', [id])
  ...
  try {
    ...
    const result = await query(
      `UPDATE payment_methods 
       SET name = ?, code = ?, type = ?, logo_url = ?, description = ?, 
           fee_percentage = ?, fee_fixed = ?, is_active = ?, position = ?, 
...
export async function DELETE(request, { params }) {
  try {
    ...
    const result = await query('DELETE FROM payment_methods WHERE id = ?', [id])
