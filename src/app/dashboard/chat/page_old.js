  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setNewMessage('')
        fetchChatUsers() // Refresh user list to update last message
      } else {
              ...
              >
                ...
                <div className="flex-1 text-left min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    {user.unread_count > 0 && (
                      <span className="ml-2 bg-violet-600 text-white text-xs font-bold rounded-full px-2 py-0.5">
                        {user.unread_count}
                      </span>
                    )}
                  </div>
          ...
          <>
                  ...
                  <div key={msg.id} className={`flex ${isOwn ? 'justify-end' : 'justify-start'} items-start space-x-3`}>
                    {!isOwn && (
                      selectedUser.avatarBase64 ? (
                        ...
                        <div className="w-10 h-10 flex items-center justify-center rounded-full bg-violet-200 text-violet-700 font-bold flex-shrink-0">
            ...
            <div className={`p-4 ${isDarkMode ? 'border-t border-gray-700' : 'border-t border-gray-200'}`}>
              <form onSubmit={handleSendMessage} className="flex flex-col space-y-2">
                {/* Instructions */}
                <div className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'} flex items-center space-x-2`}>
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <span>
                    {sending ? 'Sending message...' : `Chatting with ${selectedUser.name} (${selectedUser.role === 'admin' || selectedUser.role === 'ADMIN' ? 'Admin' : 'Reseller'})`}
                  </span>
                </div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <button
                    ...
                    className="p-3 rounded-lg bg-violet-600 hover:bg-violet-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center space-x-2"
